    ...
    <div class="conter_box">
        <div class="middle_box">
            ...
            <form>
                ...
                <dl>
                    <dt class="width5rem">选择套餐<span>*</span></dt>
                    <dd id="radios">
                        <?php foreach ($page['info']['order']['packages'] as $k => $v) { ?>
                            <label><input type="radio" name="cheage1" <?php echo $k == 0 ? "checked" : " "; ?>
                                          value="<?php echo $v['package_price'] ?>"
                                          datan="<?php echo $v['package_name'] ?>"
                                          dataId="<?php echo $v['package_id'] ?>"><?php echo $v['package_name'];
                                if ($v['recommend'] == 1) {
                                    ?>
                                    <img
                                            src="<?php echo $page['info']['cdn_url']; ?>/static/order/images/important.gif">
                                <?php } ?>
                            </label>
                        <?php } ?>
                ...
                <dl>
                    <dt class="width5rem">价格<span>*</span></dt>
                    <dd>
                        <span class="prices"> <en id="yuanpopu"></en>元</span>
                    </dd>
                </dl>
...
<script src="<?php echo $page['info']['css_cdn_url']; ?>/static/order/js/jquery.area.js"></script>
...
<script>
    (function () {
        ...
        $(document).ready(function () {
            ...
            /*获取默认选中套餐的价钱*/
            for (var i = 0; i < $('#radios :radio').length; i++) {
                if ($('#radios :radio')[i].checked) {
                    $('#yuanpopu').html($('#radios :radio')[i].value)
            ...
            /*选择套餐的价格显示*/
            $(document).on('click', '#popus_goumai :radio', function () {
                $('#yuanpopu').html($(this).val())
            });
            /*购买信息提交*/
...
//                                $('#liuyanpopus').val('');//用户留言
                                checkOrderCookie();
//                            window.open("http://test2.pinsetang.net/alipay/wap.php")
//                                location.href = "http://test2.pinsetang.net/orderApi/payment/alipay/wap.php?payAmount=" + submarry[1] + "&orderName=" + submarry[0] + '&orderCode=' + ret['orderCode'];
                                $('#success_info').show();
                                submarry = [];
                            } else {
                                $("#returnpopus").show();
