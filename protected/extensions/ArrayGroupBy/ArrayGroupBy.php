...
class ArrayGroupBy
{
    ...
    {
        if (!is_array($arr)) {
            trigger_error('self::groupByFieldDeep(): The first argument should be an array', E_USER_ERROR);
        }
        if (!is_string($key) && !is_int($key) && !is_float($key)) {
            trigger_error('self::groupByFieldDeep(): The key should be a string or an integer', E_USER_ERROR);
        }
        ...
        if (func_num_args() > 3) {
            ...
            foreach ($grouped as $key => $value) {
                ...
                $grouped[$key] = call_user_func_array("self::groupByFieldDeep", $params);
            }
        }
        return $grouped;
    }
    ...
    public static function getDeepestArray($array)
    {
        ...
        foreach ($array as $arr) {
            if (!is_array($arr)) {
                ...
                $result = array_merge(array_values($result), call_user_func("self::getDeepestArray", $arr));
            }
        }
    ...
    public static function arrayColumn($array, $column_key, $index_key = null)
    {
        if (!is_array($array) && !($array instanceof \ArrayAccess))
            throw new \Exception('Argument 1 passed to self::::arrayColumn() must be of the type array');
