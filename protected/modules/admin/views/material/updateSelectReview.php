...
<div class="main mhead">
    <div class="snav">素材管理 » 评论管理 » <?php echo $page['info']['id'] ? '修改精选评论' : '添加精选评论' ?></div>
</div>
<div class="main mbody">
    <form method="post"
          action="<?php echo $this->createUrl($page['info']['id'] ? 'material/editSelectReview' : 'material/addSelectReview'); ?> ">
        <input type="hidden" id="id" name="id" value="<?php echo $page['info']['id']; ?>"/>
        <input type="hidden" id="review_type" name="review_type" value="精品评论"/>
        <table class="tb3" id="SelectReview">
            <tbody>
            <tr>
            ...
            <?php if (!$page['info']['id']) { ?>
                <tr>
                    <td>
                        <div class="review">&nbsp;&nbsp;1楼：<span class="fewer" onclick="showHide(this)">收起</span></div>
                                        ...
                                        <textarea style="width:360px; height: 110px" id="info_body_1"
                                            ...
                                            <script>
                                                $("#info_body_1").xheditor({
                                                    plugins: allplugin,
                                                    internalScript: true,
                                                    tools: "FontSize,Fontface,Bold,FontColor,Img,Removeformat",
                                                    skin: "nostyle"
                                                });
                                            </script>
            ...
            <?php } else {
                foreach ($page['review_data'] as $key => $val) {
                    ...
                    <tr>
                        <td>
                            <div class="review">&nbsp;&nbsp;<?php echo($val[0]['floor']); ?>楼：<?php if($val[0]['floor']!=1){?><span onclick="deleteTr(this);" data-val="<?php echo ($val[0]['floor'] + 1);?>" class="deleteTr" >删除</span><?php }?><span class="fewer" onclick="showHide(this)">收起</span>
                            </div>
                            <div id="review<?php echo $val[0]['floor']; ?>"
                                 style="border: 1px solid black;height: <?php echo $count * 220; ?>px;width: 600px;margin: 0;padding: 0;">
                                <table style="margin: 0 20px;border: none;">
                                    <tr>
                                        <td width="70px">名称</td>
                                        <td>
                                    ...
                                    <?php if ($count > 1) {
                                        for($i=1;$i<$count;$i++){
                                            ...
                                            <tr class="<?php echo $review_id?>">
                                                <td colspan="3"
                                                    style="background-color: #CCCCCC;">|作者回复<span onclick="deleteTd(this);" data-floor="<?php echo $val[$i]['floor'];?>" data-val="<?php echo $floor; ?>" title="删除回复" class="deleteTd">X</span></td>
                                            </tr>
                                            <tr class="<?php echo $sign." ".$review_id; ?>">
                                                <td>
                                                    <input hidden type="text" name="review_name[<?php echo $val[$i]['floor']; ?>][]"value="作者" />
...
<script>
    ...
    function addReply(e) {
        ...
        var addHtml = '<tr class="'+ review_id +'">' +
            '<td  colspan="3" style="background-color: #CCCCCC;">' +'|作者回复' + '<span onclick="deleteTd(this);" data-floor="'+floor_num+'" data-val="' + (c + 1) + '" title="删除回复" class="deleteTd">X</span></td></tr>' +
            '<tr class="' + sign + ' '+ review_id +'"></tr>' +
            '<tr class="'+ review_id +'"><td>回复内容</td><td>' +
            '<input hidden name="reply_to[' + floor_num + '][]" value="' + data_user + '"  />' +
            '<input hidden  type="text" name="review_name[' + floor_num + '][]"value="作者" />'+
            '<div style="position:relative;">' +
    ...
    $(function () {
        $("#add_review").click(function () {
            ...
            var addHtml = '<tr><td>' +
                '<div class="review">&nbsp;&nbsp;' + (b) + '楼：<span onclick="deleteTr(this);" data-val="' + (b + 1) + '" class="deleteTr" >删除</span><span class="fewer" onclick="showHide(this);" >编辑</span></div> ' +
                ...
                '</span> </div></td>' +
                '<td>  <input type="button" class="but" id="reply" data-val="' + b + '" data-user="0"onclick="addReply(this)"value="作者回复"/></td>' +
                '</tr><tr>' +
                '<td>楼层时间</td>' +
                '<td>' +
                ' <input style="width: 200px;height:30px;font-size: large;" type="text" class="ipt" ' +
