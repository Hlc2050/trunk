...
<div class="main mhead">
    <div class="snav">计划管理 » 每周进粉计划 » 个人计划修改</div>
</div>
<div class="main mbody">
    <form method="post"
          action="<?php echo $this->createUrl('planWeek/editUserPlan'); ?>" onsubmit="return chckForm()">
        ...
        <table class="tb3" style="border: solid 1px #c2c2c2;width: 1000px" id="plan_table">
            ...
            <tr>
                ...
                <td colspan="2"></td>
                <td colspan="6"><?php echo $page['user_name'];?>-<?php echo date('Y-m-d', $page['week_dates'][0]) . '至' . date('Y-m-d', end($page['week_dates'])) ?>
                    -排期计划
                </td>
            </tr>
            <tr>
                ...
                <tr class="plan_tr">
                    <td style="text-align: center;border: solid 1px #CCCCCC" >
                        <?php if ($page['week_plan']['start_date'] <$today) {?>
                            <select name="cservice_group[]" style="width:100px;margin-top: 8%" readonly>
                    ...
                    <?php foreach ($value as $k=>$plan) { ?>
                        <td style="text-align: center;border: solid 1px #CCCCCC" >
                            <input type="text" name="wechat_<?php echo $key;?>[]" value="<?php echo $plan['weChat_num'];?>" style="width: 60px;margin-bottom: 5px;" <?php if($k < $today){ echo "readonly" ;}?> required><br/>
                                <input type="text" style="width: 60px;margin-bottom: 5px;" name="fans_<?php echo $key;?>[]" value="<?php echo $plan['fans_count'];?>" <?php if($k < $today){ echo "readonly" ;}?> required><br/>
            ...
            <tr id="add_tr_info" style="display: none">
                ...
                <?php for ($i=0;$i<7;$i++) { ?>
                    <td style="text-align: center;border: solid 1px #CCCCCC">
                        <input type="text" name="wechat"  style="width: 60px;margin-bottom: 5px;" value="<?php if($page['week_dates'][$i] < $today){ echo '0' ;}?>" <?php if($page['week_dates'][$i] < $today){ echo "readonly" ;}?> ><br/>
                        <input type="text" name="fans" style="width: 60px;margin-bottom: 5px;"  value="<?php if($page['week_dates'][$i] < $today){ echo '0' ;}?>"   <?php if($page['week_dates'][$i] < $today){ echo "readonly" ;}?> ><br/>
                        <input type="text" name="output"  style="width: 60px;margin-bottom: 5px;" value="<?php if($page['week_dates'][$i] < $today){ echo '0' ;}?>" <?php if($page['week_dates'][$i] < $today){ echo "readonly" ;}?> >
                    </td>
                <?php } ?>
                <td style="text-align: center;">
