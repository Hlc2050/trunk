...
<div class="main mhead" xmlns="http://www.w3.org/1999/html">
    <div class="snav">财务管理 »  客服部投放	</div>

    <div class="mt10">
        <form action="<?php echo $this->createUrl('customerPut/index'); ?>">日期：
            ...
            商品：
            <?php
            echo  CHtml::dropDownList('goods_id', $this->get('goods_id'),CHtml::listData(Goods::model()->findAll(), 'id', 'goods_name'), array('empty' =>'全部'))
            ?>&nbsp;&nbsp;
    ...
    <div class="main mbody">
        <form action="<?php echo $this->createUrl('customerPut/saveOrder'); ?>" name="form_order" method="post">
                ...
                foreach($page['listdata']['list'] as $r){
                    ...
                    <tr>
                        ...
                        for ($i=0;$i<$total_show;$i++){
                            ...
                            $sql_detail="select SUM(money) as money from stat_cost_detail WHERE customer_service_id={$r['customer_service_id']} AND stat_date BETWEEN $date_s AND $date_e".$condition;
                            $sql_fixed="select SUM(fixed_cost) as fixed_cost from fixed_cost_new WHERE customer_service_id={$r['customer_service_id']} AND stat_date BETWEEN $date_s AND $date_e".$condition;
                            $cost_detail=Yii::app()->db->createCommand($sql_detail)->queryAll();
                            $cost_fixed=Yii::app()->db->createCommand($sql_fixed)->queryAll();
                            $cost_customer=0;
                            $cost_customer = $cost_detail[0]['money'] + $cost_fixed[0]['fixed_cost'];
                            $cost_customer=round($cost_customer,2);
                            echo '<td>'.$cost_customer.'</td>';
                            $sum_cost = $sum_cost+$cost_customer;
                        }
                        ?>
