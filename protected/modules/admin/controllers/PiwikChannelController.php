 ...
 * Time: 11:29
 */
class PiwikChannelController extends AdminController{
    public function actionIndex(){
        ...
        $params['select']="  a.*,c.unit_price,c.charging_type,c.sno,c.weixin_group_id,d.id as cid,d.channel_name,d.channel_code,e.name  ";
    ...
    public function actionExport()
    {
        ...
        foreach ($data as $key => $val) {
            if ($n % $page_size === 0) {
                ...
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('C1','合作商');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(15);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('D1','渠道名称');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(15);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('E1','渠道编码');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(15);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('F1','计费单价');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(15);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('G1','计费方式');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(20);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('H1','pv');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('I')->setWidth(15);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('I1','uv');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('J')->setWidth(15);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('J1','独立ip');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('K')->setWidth(15);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('K1','微信号长按次数');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('L')->setWidth(20);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('L1','二维码长按次数');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('M')->setWidth(20);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('M1','第三方投入金额');
                $objectPHPExcel->getActiveSheet()->getColumnDimension('N')->setWidth(20);
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('N1','微信号小组');

                //设置居中
                $objectPHPExcel->getActiveSheet()->getStyle('A1:N1')
                ...
                //设置颜色
                $objectPHPExcel->getActiveSheet()->getStyle('A1:N1')->getFill()
                    ->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FF66CCCC');
            }
            //设置居中
            $objectPHPExcel->getActiveSheet()->getStyle('A:N')
                ->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            //明细的输出
            ...
            $objectPHPExcel->getActiveSheet()->setCellValue('C' . ($n + 2), $val['name']);
            $objectPHPExcel->getActiveSheet()->setCellValue('D' . ($n + 2), $val['channel_name']);
            $objectPHPExcel->getActiveSheet()->setCellValueExplicit('E' . ($n + 2), $val['channel_code'],PHPExcel_Cell_DataType::TYPE_STRING);
            $objectPHPExcel->getActiveSheet()->setCellValueExplicit('F' . ($n + 2), $val['unit_price'],PHPExcel_Cell_DataType::TYPE_STRING);
            $objectPHPExcel->getActiveSheet()->setCellValue('G' . ($n + 2), $val['charging_type']);
            $objectPHPExcel->getActiveSheet()->setCellValue('H' . ($n + 2), $val['pv']);
            $objectPHPExcel->getActiveSheet()->setCellValue('I' . ($n + 2), $val['uv']);
            $objectPHPExcel->getActiveSheet()->setCellValue('J' . ($n + 2),  $val['ip']);
            $objectPHPExcel->getActiveSheet()->setCellValue('K' . ($n + 2), $val['wechat_touch']);
            $objectPHPExcel->getActiveSheet()->setCellValue('L' . ($n + 2), $val['qr_code_click']);
            $objectPHPExcel->getActiveSheet()->setCellValue('M' . ($n + 2), $val['cost']);
            $objectPHPExcel->getActiveSheet()->setCellValue('N' . ($n + 2), $val['wechat_group_name']);
            //设置边框
            $currentRowNum = $n + 2;
            $objectPHPExcel->getActiveSheet()->getStyle('A' . ($n + 2) . ':N' . $currentRowNum)
                ->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
            $objectPHPExcel->getActiveSheet()->getStyle('A' . ($n + 2) . ':N' . $currentRowNum)
                ->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
            $objectPHPExcel->getActiveSheet()->getStyle('A' . ($n + 2) . ':N' . $currentRowNum)
        ...
        header('Content-Disposition:attachment;filename="' . '第三方统计-渠道统计-' . date("Ymj") . '.xls"');
     ...
     * author:
     */
    private function GetPiwikChannelData()
    {
        $params['where'] = '';
        //strtotime($_GET['pay_date']);
        if ($this->get('start_date') != '' && $this->get('end_date') != '') {
