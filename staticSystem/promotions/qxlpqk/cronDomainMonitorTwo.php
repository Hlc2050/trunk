...
//            $this->dbm->doSql($sql);
        ...
        if ($iswarming == 1) {
            ...
            foreach ($openids as $openid) {
                //仅当被拦截或被拦截域名恢复才通知
                $this->sendEvent($openid, "二部域名监控(Line-two)", $template_desc, date('Y-m-d H:i:s', time()), $this->config['domain']['show_domain_url'] . "/?start=" . $check_start_time . "&end=" . $check_end_time. "&line=2", $template_id, $access_token); //发送报警通知
            }
        }
//        $this->domain_intercept_clear();
    }
    ...
    {
        $url = 'http://vip.weixin139.com/weixin/wx_domain.php?user=qaoqq2&key=bfaeeed1d4956a8a4759e1261ff2c0b0&domain=' . $domain;
        $ch = curl_init();
    ...
    private function domain_list()
    {
        ...
        $sql = "SELECT * FROM `domain_list` WHERE status=1 and promotion_type!=1 and mod(id,2) = 0 and id NOT IN (".$pauseIds.") ORDER BY `id` DESC";
