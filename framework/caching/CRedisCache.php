...
class CRedisCache extends CCache
{
    ...
    protected function connect()
    {
        ...
        if ($this->_socket) {
            ...
            $info = Params::model()->find("param_name='is_redis'");
            $info->param_value=0;
            $info->save();
            $file = dirname(__FILE__) . "/../../protected/config/params.php";
            $config = array();
...
//throw new CException('Failed to connect to redis: ' . $errorDescription, (int)$errorNumber);
    ...
    {
        if (($line = fgets($this->_socket)) === false)  return null;
...
// no cast to int as it is in the range of a signed 64 bit integer
    ...
    {
        $response = $this->executeCommand('keys', array($key));
        return $response;
    }
    ...
    public function deleteValues($key)
    {
    ...
    /**
     *   队列操作
     *    lpush  添加一个字符串值到LIST容器的顶部（左侧），如果KEY不存在，创建一个LIST容器。如果KEY存在并且不是一个LIST容器，那么返回FLASE。
     */
    public function lpush($key, $value) {
        return $this->executeCommand('lpush', array($key, serialize($value)));
    }

    /**   队列操作
     *    lpop 返回LIST顶部（左侧）的VALUE，并且从LIST中把该VALUE弹出。
     */
    public function lpop($key) {
        return unserialize($this->executeCommand('lpop', array($key)));
    }

    /**
     *   队列操作
     *    rpush  添加一个字符串值到LIST容器的底部（右侧），如果KEY不存在，创建一个LIST容器，如果KEY存在并且不是一个LIST容器，那么返回FLASE。 
     */
    public function rpush($key, $value) {
        return $this->executeCommand('rpush', array($key, serialize($value)));
    }

    /**   队列操作
     *     rpop 返回LIST底部（右侧）的VALUE，并且从LIST中把该VALUE弹出。
     */
    public function rpop($key) {
        return unserialize($this->executeCommand('rpop', array($key)));
    }
