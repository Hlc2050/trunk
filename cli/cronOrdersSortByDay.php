...
class ordersSortByDay
{
    ...
    public function run()
    {
        $stat_date = date('Ymd', strtotime('-1 day'));
        $yesterdayWxData = $this->get_yesterday_wx_data($stat_date);

        if ($yesterdayWxData) {
            $table_name='orders_sort_by_day';
            $rows=array_keys(reset($yesterdayWxData));
            $ret=$this->batch_insert_data($table_name,$rows,$yesterdayWxData);
        }
     ...
     * author: yjh
     */
    private function get_yesterday_wx_data($stat_date)
    {

        //获取当天进线数据，除了发货量和发货金额
        $sql = 'SELECT b.promotion_id,b.partner_id,b.channel_id,b.customer_service_id,a.weixin_id,a.wechat_id,c.package_id,goods_id,business_type,charging_type,promotion_staff_id as tg_uid,count(*) AS in_count
                 ...
                 group by a.weixin_id,b.channel_id';
        ...
        $sql = 'SELECT b.promotion_id,b.partner_id,b.channel_id,b.customer_service_id,a.weixin_id,a.wechat_id,c.package_id,goods_id,business_type,charging_type,promotion_staff_id as tg_uid,count(*) AS sout_count,sum(c.real_price) as sdelivery_money
                 from order_assistant as a left join order_manage as b on a.order_id=b.id left join order_goods_manage as c on a.order_id=c.order_id
                 where order_status=1 and business_type='.$this->dx_bid.' and o_date = ' . $stat_date . '
                 group by a.weixin_id,b.channel_id';
        $sInfo = $this->order_dbm->doSql($sql);
        //获取发货量以及发货金额
        ...
        if ($inInfo) {
            foreach ($inInfo as $key => $value) {
                if (array_key_exists($key, $sInfo)) {
                    $inInfo[$key]['sout_count'] = $sInfo[$key]['sout_count'];
                    $inInfo[$key]['sdelivery_money'] = $sInfo[$key]['sdelivery_money'];
                } else {
                    $inInfo[$key]['sout_count'] = 0;
                    $inInfo[$key]['sdelivery_money'] = 0;
                }
